...
const ComponentCreateSubPlan = React.forwardRef<
...
>(({ className, onValueChange, ...props }, ref) => {
  ...
  //     try {
  //       if (id) {
  //         const result = await subscriptionPlanApiRequest.subscriptionPlanClient(id);
  //         setSubPlan(result.payload);
  //         // console.log(result);

  //         // Update form values with fetched chatbot data
  //         form.setValue("plan_title", result.payload.plan_title || "");
  //         form.setValue("plan_price", result.payload.plan_price || 0);
  //         form.setValue("message_credits", result.payload.message_credits || 0);
  //         form.setValue(
  //           "max_character_per_chatbot",
  //           result.payload.max_character_per_chatbot || 0
  //         );
  //         form.setValue(
  //           "number_of_chatbots",
  //           result.payload.number_of_chatbots || 0
  //         );
  //         form.setValue(
  //           "available_model",
  //           result.payload.available_model || ""
  //         );
  //         form.setValue("remove_label", result.payload.remove_label || false);
  //       }
  //     } catch (error) {
  //       handleErrorApi({
  //         error,
  ...
  async function onSubmit(values: EditSubscriptionPlanBodyType) {
    ...
    setLoading(true);
    try {
      const result = await subscriptionPlanApiRequest.createSubscriptionPlan(
        values
      );
      toast({
        title: "Success",
...
ComponentCreateSubPlan.displayName = SliderPrimitive.Root.displayName;
export { ComponentCreateSubPlan };
